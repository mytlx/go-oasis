	...
	return func(c *gin.Context) {
		...
		if _, ok := pool.Get(rid); ok {
			c.String(http.StatusBadRequest,
				fmt.Sprintf("房间[%s]已存在，请访问：http://localhost:8090/bili/%s/index.m3u8", rid))
			return
		}
...
func RoomDetailHandler(pool *pool.ManagerPool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.String(http.StatusOK, fmt.Sprintf("http://localhost:8090/bilibili/%s/index.m3u8", manager.Get().Id))
	}
}
