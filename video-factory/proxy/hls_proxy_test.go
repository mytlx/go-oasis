...
func TestUrlResolve(t *testing.T) {
	...
	targetRequestURL := resolveUrl(hlsUrl, "/bili/524833285.m4s")
	log.Printf("目标URL: %s\n", targetRequestURL.String())

	targetRequestURL2 := resolveUrl(hlsUrl, "/bili/manager_524833285.m3u8")
...
func resolveUrl(hlsUrl, requestUrl string) *url.URL {
	...
	// 目标URL的路径部分替换为客户端请求的路径，去掉 /bili 前缀
	requestPath := strings.TrimPrefix(requestUrl, "/bili")
